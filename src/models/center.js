...
import { CENTER_NAMESPACE } from '../actions/center';
import { query as queryCenters, list, submit, update, detail, remove, grant } from '../services/center';
...
import { getCurrentCenter } from '../utils/authority';
...
export default {
  ...
  effects: {
    *fetch(_, { call, put }) {
      const response = yield call(queryCenters);
      yield put({
        type: 'save',
        payload: response,
      });
  ...
  reducers: {
    ...
    saveCurrentCenter(state, action) {
      return {
        ...state,
