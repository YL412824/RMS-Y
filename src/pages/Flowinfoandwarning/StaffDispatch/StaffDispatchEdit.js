...
import { STAFFDISPATCH_DETAIL, STAFFDISPATCH_SUBMIT } from '../../../actions/staffdispatch';

const FormItem = Form.Item;

@connect(({ staffdispatch, loading }) => ({
  staffdispatch,
  submitting: loading.effects['staffdispatch/submit'],
}))
@Form.create()
class StaffDispatchEdit extends PureComponent {
  componentWillMount() {
    const {
      dispatch,
      match: {
  ...
  handleSubmit = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        console.log(params);
        dispatch(STAFFDISPATCH_SUBMIT(params));
      }
    });
  };
  ...
  render() {
    ...
    return (
      <Panel title="修改" back="/tool/staffdispatch" action={action}>
...
export default StaffDispatchEdit;
