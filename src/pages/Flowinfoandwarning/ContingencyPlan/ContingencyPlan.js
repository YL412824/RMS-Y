...
import { CONTINGENCYPLAN_INIT, CONTINGENCYPLAN_LIST, CONTINGENCYPLAN_ROLE_GRANT } from '../../../actions/contingencyplan';
import { resetPassword } from '../../../services/contingencyplan';
...
class ContingencyPlan extends PureComponent {
  ...
  handleSearch = params => {
    ...
    const { dispatch } = this.props;
    dispatch(CONTINGENCYPLAN_LIST(params));
  };

  // ============ 处理按钮点击回调事件 ===============
  handleBtnCallBack = payload => {
    const { btn, keys } = payload;
    if (btn.code === 'contingencyplan_role') {
      if (keys.length === 0) {
    ...
    if (btn.code === 'contingencyplan_reset') {
      if (keys.length === 0) {
        message.warn('请先选择一条数据!');
        return;
      }
      Modal.confirm({
        ...
        cancelText: '取消',
        async onOk() {
          const response = await resetPassword({ contingencyplanIds: keys });
          if (response.success) {
  ...
  handleGrant = () => {
    ...
    const { dispatch } = this.props;
    dispatch(
      CONTINGENCYPLAN_ROLE_GRANT({ contingencyplanIds: keys, roleIds: checkedTreeKeys.checked }, () => {
        this.setState({
          visible: false,
          confirmLoading: false,
        });
  ...
  handleExport = () => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        window.open(
          `/api/blade-contingencyplan/export-contingencyplan?blade-auth=${getAccessToken()}&account=${account}&realName=${realName}`
        );
      },
      onCancel() {},
    });
  ...
  handleTemplate = () => {
    window.open(`/api/blade-contingencyplan/export-template?blade-auth=${getAccessToken()}`);
  };
