...
import { DATASHOW_DETAIL, DATASHOW_SUBMIT } from '../../../actions/datashow';

const FormItem = Form.Item;

@connect(({ datashow, loading }) => ({
  datashow,
  submitting: loading.effects['datashow/submit'],
}))
@Form.create()
class DatashowEdit extends PureComponent {
  componentWillMount() {
    const {
      dispatch,
      match: {
  ...
  handleSubmit = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        dispatch(DATASHOW_SUBMIT(params));
      }
    });
  };
  ...
  render() {
    ...
    return (
      <Panel title="修改" back="/system/datashow" action={action}>
        <Form hideRequiredMark style={{ marginTop: 8 }}>
          <Card className={styles.card} bordered={false}>
            <Row gutter={24}>
              <Col span={10}>
                <FormItem {...formItemLayout} label="客户端id">
                  {getFieldDecorator('datashowId', {
                    rules: [
                      {
                        required: true,
                        message: '请输入客户端id',
                    ...
                    initialValue: detail.datashowId,
                  })(<Input placeholder="请输入客户端id" />)}
                </FormItem>
              </Col>
              <Col span={10}>
                <FormItem {...formItemLayout} label="客户端密钥">
                  {getFieldDecorator('datashowSecret', {
                    rules: [
                      {
                        required: true,
                        message: '请输入客户端密钥',
                    ...
                    initialValue: detail.datashowSecret,
