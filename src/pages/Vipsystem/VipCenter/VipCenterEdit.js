...
import { VIPCENTER_CHANGE_INIT, VIPCENTER_DETAIL, VIPCENTER_UPDATE } from '../../../actions/vipcenter';
...
@connect(({ vipcenter, loading }) => ({
  vipcenter,
  submitting: loading.effects['vipcenter/submit'],
}))
@Form.create()
class VipCenterEdit extends PureComponent {
  componentWillMount() {
    const {
      dispatch,
      match: {
    ...
    dispatch(VIPCENTER_DETAIL(id)).then(() => {
      const {
        vipcenter: { detail },
      } = this.props;
      dispatch(VIPCENTER_CHANGE_INIT({ tenantId: detail.tenantId }));
    });
  }
  ...
  handleSubmit = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        dispatch(VIPCENTER_UPDATE(params));
      }
    });
  };
  ...
  handleChange = value => {
    ...
    dispatch(VIPCENTER_CHANGE_INIT({ tenantId: value }));
  };
  ...
  render() {
    ...
    return (
      <Panel title="修改" back="/system/vipcenter" action={action}>
