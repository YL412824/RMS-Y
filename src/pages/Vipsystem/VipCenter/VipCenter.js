...
import { VIPCENTER_INIT, VIPCENTER_LIST, VIPCENTER_ROLE_GRANT } from '../../../actions/vipcenter';
import { resetPassword } from '../../../services/vipcenter';
...
@connect(({ vipcenter, loading }) => ({
  vipcenter,
  loading: loading.models.vipcenter,
}))
@Form.create()
class VipCenter extends PureComponent {
  state = {
    visible: false,
    excelVisible: false,
  ...
  handleSearch = params => {
    ...
    const { dispatch } = this.props;
    dispatch(VIPCENTER_LIST(params));
  };

  // ============ 处理按钮点击回调事件 ===============
  handleBtnCallBack = payload => {
    const { btn, keys } = payload;
    if (btn.code === 'vipcenter_role') {
      if (keys.length === 0) {
    ...
    if (btn.code === 'vipcenter_reset') {
      ...
      Modal.confirm({
        ...
        cancelText: '取消',
        async onOk() {
          const response = await resetPassword({ vipcenterIds: keys });
          if (response.success) {
  ...
  handleGrant = () => {
    ...
    const { dispatch } = this.props;
    dispatch(
      VIPCENTER_ROLE_GRANT({ vipcenterIds: keys, roleIds: checkedTreeKeys.checked }, () => {
        this.setState({
          visible: false,
          confirmLoading: false,
        });
  ...
  handleExport = () => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        window.open(
          `/api/blade-vipcenter/export-vipcenter?blade-auth=${getAccessToken()}&account=${account}&realName=${realName}`
        );
      },
      onCancel() {},
    });
  ...
  handleTemplate = () => {
    window.open(`/api/blade-vipcenter/export-template?blade-auth=${getAccessToken()}`);
  };
  ...
  render() {
    ...
    const uploadProps = {
      ...
      action: '/api/blade-vipcenter/import-vipcenter',
    };
