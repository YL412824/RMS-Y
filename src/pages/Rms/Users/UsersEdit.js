...
import { USERS_CHANGE_INIT, USERS_DETAIL, USERS_UPDATE } from '../../../actions/users';
...
class UsersEdit extends PureComponent {
  componentWillMount() {
    ...
    dispatch(USERS_DETAIL(id)).then(() => {
      const {
        users: { detail },
      } = this.props;
      dispatch(USERS_CHANGE_INIT({ tenantId: detail.tenantId }));
    });
  }
  ...
  handleChange = value => {
    ...
    dispatch(USERS_CHANGE_INIT({ tenantId: value }));
  };
  ...
  render() {
    ...
    return (
      <Panel title="修改" back="/system/users" action={action}>
